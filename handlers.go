...
func (a *apiConfig) createUserHandler(w http.ResponseWriter, r *http.Request) {
	...
	newUser, err := a.DB.CreateUser(user.Email)
	if err != nil {
...
func (a *apiConfig) getUsersHandler(w http.ResponseWriter, _ *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusInternalServerError, "Could not retrieve users from database")
	}
...
func (a *apiConfig) getUserByIdHandler(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		respondWithError(w, http.StatusNotFound, "No user found in database")
		return
	}
